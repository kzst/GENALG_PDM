...
%Agile Project Management agent (APMa)
%----------------
%Standard output:
...
function target=targetfcn(chromosome)
...
A=arrayfun(@(x) updatepem(x.f1), s,'UniformOutput',false); %set of DSMs
for i=1:PopSize
    tpts(i)=tptfast(A{i},T);
    tpcs(i)=diag(A{i})'*C;
    c(i)=max([tpts(i)-Ct,tpcs(i)-Cc,Cs-maxscore_PEM(A{i},P,Q)]);
...
switch typ
    ...
    case 2 %minimize TPC
        target=tpcs+c';
    case 3 %minimize TPS
        target=-(maxscore_PEM_cell(A,P,Q)'*100+maxscore_SNPM_cell(A,P,Q)')+c';
