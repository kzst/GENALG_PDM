...
%----------------
%Generate a PDM matrix for TCTP problems
%----------------
%Output:
...
%---------------- 
%Usage:
%PDM=generatepdm(N,ff,cf,mTD,mCD,mRD,w,nR,nW)
%---------------- 
%Example:
%PDM=generatepdm(30,0.05,0,20,30,20,2,2,2)

function PDM=generatepdm(N,ff,cf,mTD,mCD,mRD,w,nR,nW)
cf=cf+1;
PEM=phase3(triu(triu(min(ones(N)./max(repmat([1-cf:N-cf],N,1).^1.4-...
    (repmat([0:N-1]',1,N).^1.4),ones(N)),ones(N)),1)>rand(N),1)+...
