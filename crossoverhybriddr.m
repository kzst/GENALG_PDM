...
%----------------
%Crossover function for resource-constrained HDTCTPs 
%----------------
%Standard output:
...
%---------------- 
%Usage:
%xoverKids = crossoverhybriddr(parents,options,GenomeLength,FitnessFcn,...
...
function xoverKids  = crossoverhybriddr(parents,options,GenomeLength,...
...
for i=1:nKids
    ...
    % (MODES) and the vector of scheduled start times (SST)
    PSM1=updatepemdr(thisPopulation(r1,:));
    DSM1=PSM1(:,1:size(PSM1,1));
    MODES1=PSM1(:,end-1);
    SST1=PSM1(:,end)';
    PSM2=updatepemdr(thisPopulation(r2,:));
    ...
    xoverKids(i,:)=crossdsmdr(DSM1,MODES1,SST1,DSM2,MODES2,SST2);
end
