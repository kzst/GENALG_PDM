...
function PSM=hpmgench(PDM,const,typefcn)
...
if Typ==1 %When minimizing TPT ...
    ...
    PSM=[PSM,T,timetocost(PSM(:,end),T,C)]; %hpmgench===hpmgenc
end
[~,EST]=tptfast(PSM(:,1:N),PSM(:,N+1)); %Calculate EST...  
PSM=[PSM,EST];                          %for the last column of PSM matrix
