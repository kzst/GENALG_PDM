...
%allocation for a continouos hybrid multi-objective multi-mode resource 
%constraint project scheduling problem ((c)HMOMRCPSP) by hybrid genetic 
%algorithm 
%----------------
%Output:
...
%---------------- 
%Usage:
%PSM=parhpmgencqr(PDM,const)
%---------------- 
%Example:
%PDM=[triu(rand(10)*.5+.5),20*rand(10,2),30*rand(10,2),rand(10,2),5*...
%   rand(10,6)];
%const=[percentt(PDM,2,.9),percentc(PDM,2,.9),percentq(PDM,2,.7),...
%  percentqr(PDM,2,1)',percents(PDM,.7)];
%tic;PSM=parhpmgencqr(PDM,const);toc
%---------------- 
...
function PSM=parhpmgencqr(PDM,const)
global PEM P Q T C q R Ct Cc CR Cq Cs QD
%global values: 
...
%running GA.
% CrossoverFcn:@crossoverhybridcqr specifies a unique crossoverhybridcqr 
...
options=gaoptimset('Display','off','PopulationSize',PopSize,...
    ...
    'CrossoverFcn',@crossoverhybridcqr,...
...
LST(LST<0)=0;
LB=[zeros(1,numel(PEM(PEM>0&PEM<1))),min(TD,[],2)',EST'];      %Lower Bound
UB=[ones(1,numel(PEM(PEM>0&PEM<1))),max(TD,[],2)',LST'];       %Upper Bound

%---End of setting GA---

%---Runing GA---
% @targetfcncqr: the target function (=targetfcncqr)
% n+2*N: the number of variables (n=number of uncertainties + N=number of
%activities
% LB,UB: Lower/Upper Bounds
% @constrainrtparcqr: The nonlinear constraint function
...
X=gamultiobj(@targetfcncqrpar,n+2*N,[],[],[],[],LB,UB,...
    @constrainrtparcqr,options);
...
PSM=pmtopsmcqr(PSM,T,C,q,R); %The output PSM = [DSM,TD,CD,QD,RD,SST]
